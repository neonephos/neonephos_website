---
layout: CommitteeThemeComponent
title: "Governing Board"
sidebar: false
aside: false
hero:
  name: "Governing Board"
  subtitle: "The Governing Board (GB) is the strategic organizational body of NeoNephos."
  description: "It does not make technical decisions other than working with the Technical Advisory Council (TAC). The GB currently consists of these members."
members:
  - name: "Christian Neu"
    email: "main+owner@lists.neonephos.org"
    linkedin: "https://www.linkedin.com/in/christianneu/"
    imgsrc: "/assets/governing_board/christian_neu.jpg"
    role: "Chairperson & SAP Rep."
    details: "Cloud Software Architect at SAP, leading the publication of open source projects. Past work includes the Corona-Warn-App and the Kulturpass. Currently responsible for the open source publication of ApeiroRA and serves as the Governing Board Chair of the NeoNephos Foundation."
    link: "https://www.linkedin.com/in/christianneu/"
  - name: "Stephan Ilaender"
    linkedin: "https://de.linkedin.com/in/stephan-ilaender"
    imgsrc: "/assets/governing_board/stephan_ilaender.jpg"
    role: "STACKIT Rep."
    details: "As Field CTO at the Schwarz Group's cloud provider STACKIT, Stephan Ilaender has been responsible for strategic consulting and supporting customers in the digital transformation of their IT infrastructures since January 2023. A central focus of his work is the development and implementation of cloud strategies that ensure the digital sovereignty of customer workloads. This includes the design of architectures and the selection of technologies that meet data sovereignty and compliance requirements."
    link: "https://de.linkedin.com/in/stephan-ilaender"
  - name: "Christian Weiss"
    imgsrc: "/assets/governing_board/christian_weiss.jpg"
    role: "T-Systems Rep."
    details: "Christian has been with Deutsche Telekom AG since 2007, holding various leadership and innovation roles across T-Systems, Telekom Laboratories, and Technology & Innovation. His expertise spans cloud and data architecture, data management, digital identity, communication technologies, and innovation management. Christian has played key leadership roles in numerous projects, where he combines deep technical expertise with strategic business ownership to drive product development and deliver cutting-edge solutions. He is active in open-source foundations such as Eclipse and the Linux Foundation. Christian holds a PhD from the University of Bonn and a Master’s degree from the University of Heidelberg, and he was a visiting scholar at NITECH Nagoya, Japan, and INESC Lisbon, Portugal."
    link: "/"
  - name: "Christian Berendt"
    imgsrc: "/assets/governing_board/christian_berendt.jpg"
    role: "General Member Rep."
    details: "CEO of 23 Technologies."
    linkedin: "www.linkedin.com/in/christian-berendt-x3bxqftshb"
    link: "www.linkedin.com/in/christian-berendt-x3bxqftshb"

tilesHeading: Learn more!
tiles:
  - title: NeoNephos Charter
    details: Learn how our charter defines the organizational committees such as the Governing Board.
    linkText: Explore Charter
    linkUrl: https://cdn.platform.linuxfoundation.org/agreements/neonephos-foundation.pdf?__hstc=81619592.11739e1f4ca03192f4e918409f725053.1748961872693.1755169332458.1755604645613.29&__hssc=81619592.2.1755604645613&__hsfp=1879682750
  - title: Election Policy
    details: Learn about election periods and more.
    linkText: Election Policy
    linkUrl: /election_policy
  - title: Contact
    details: Want to get in touch? Contact the Governing Board via mail.
    linkText: Get In Touch
    linkUrl: mailto:main@lists.neonephos.org

---
